...
int sdp_message_init_as_default(sdp_message_t **dest,const char *host,int port,char *media,char *payloads,char *rtpmap)
{
	char strport[20];
	sdp_message_t *sdp;
	char *space;
	char *pPayloads = osip_strdup(payloads);
	char *pRtp = osip_strdup(rtpmap);
	...
	sdp_message_o_origin_set(sdp,osip_strdup("na"),osip_strdup("1234"),osip_strdup("5678")
		,osip_strdup("IN"),osip_strdup("IP4"),osip_strdup(host));

	sdp_message_s_name_set(sdp,osip_strdup("gwsip"));

	sdp_message_c_connection_add(sdp,-1,osip_strdup("IN"),osip_strdup("IP4"),osip_strdup(host),NULL,NULL);

	sdp_message_t_time_descr_add(sdp,osip_strdup("0"),osip_strdup("0"));

	if(strcmp(media,"image") == 0)
		sdp_message_m_media_add(sdp,osip_strdup(media),osip_strdup(strport),NULL,osip_strdup("udptl"));
	else
		sdp_message_m_media_add(sdp,osip_strdup(media),osip_strdup(strport),NULL,osip_strdup("RTP/AVP"));
	...
	while(1)
	{
		...
		{
			if(strlen(pay) > 0)
				sdp_message_m_payload_add(sdp,0,osip_strdup(pay));
			break;
		}
		else if(space == pay)
		{
	...
	while(1)
	{
		...
		if(space == NULL)
		{
			if(strlen(rtpmap) > 0)
			{
				char *fmt = strchr(rtpmap,':');
				if(fmt)
				{
					*fmt = '\0';
					sdp_message_a_attribute_add(sdp,0,osip_strdup(rtpmap),osip_strdup(fmt+1));
				}
				else
				{
					sdp_message_a_attribute_add(sdp,0,osip_strdup(rtpmap),NULL);
				}
			}
			break;
		}

...
sdp_init (sdp_t ** sdp)
{
  ...
  (*sdp)->c_connections = (list_t *) smalloc (sizeof (list_t));
  if((*sdp)->c_connections == NULL) return -1;
  list_init((*sdp)->c_connections);
...
sdp_parse_c (sdp_t * sdp, char *buf, char **next)
{
  ...
  *next = buf;
  ...
  if (i == 0)
  {
	  list_add(sdp->c_connections,c_header,-1);
  }
  else
    {
      sdp_media_t *last_sdp_media =
...
int rtpmap_init(rtp_map_t **map)
{
	(*map) = smalloc(sizeof(rtp_map_t));
	if(*map == NULL) return -1;
	(*map)->codec = NULL;
	(*map)->no = NULL;
	return 0;
}
void rtpmap_free(rtp_map_t *map)
{
	if(map->codec != NULL) sfree(map->codec);
	if(map->no != NULL) sfree(map->no);
}
int rtpmap_parse(rtp_map_t *rtp_map,const char *buf)
{
	char *space;
	...
	(*space) = '\0';
	rtp_map->no = sgetcopy(buf);
	rtp_map->codec = sgetcopy(space + 1);
...
sdp_2char (sdp_t * sdp, char **dest)
{
  ...
  *dest = NULL;
  ...
  pos = 0;
  while (!list_eol(sdp->c_connections,pos))
  {
	  char *next_tmp;
      
	  sdp_connection_t *conn = list_get(sdp->c_connections,pos);
      
      if( 0 != sdp_append_connection (string, size, tmp, conn,&next_tmp) ) return -1;
...
sdp_free (sdp_t * sdp)
{
  ...
  while(!list_eol(sdp->c_connections,0))
  {
	  sdp_connection_t* conn = list_get(sdp->c_connections,0);
	  list_remove(sdp->c_connections,0);
	  sdp_connection_free (conn);
	  sfree (conn);
  }

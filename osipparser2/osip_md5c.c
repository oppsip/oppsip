...
void trim(char *pScr,char ch)
{
	...
	while(*pSource)
	{
		if(*pSource == ch || *pSource == '\t' || *pSource == '\r' || *pSource == '\n')
		{
			char *pTemp = pSource;
			while(*pTemp)
			{
	...
	while(*(--pSource) == ' '); //trim the end space of pScr

	*(++pSource) = 0;
}

const char *MD5_String(const char *string,char *Dest)
{
	osip_MD5_CTX context;
	unsigned char digest[16];
	unsigned int len = strlen (string);

	osip_MD5Init (&context);
	osip_MD5Update (&context, (unsigned char *)string, len);
	osip_MD5Final (digest, &context);

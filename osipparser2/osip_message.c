...
int osip_message_init(osip_message_t ** sip)
{
	*sip = (osip_message_t *) osip_malloc(sizeof(osip_message_t));
	...
	(*sip)->from_ip = NULL;
	(*sip)->from_port = 5060;
	(*sip)->outbound_ip = NULL;
	(*sip)->outbound_port = 5060;
	
	return OSIP_SUCCESS;		/* ok */
}

osip_message_t* osip_message_addref(osip_message_t * sip)
{
	atomic_increment(&(sip->ref_count));
	return sip;
}
...
void osip_message_free(osip_message_t * sip)
{
	...
	ref = atomic_get_value(&(sip->ref_count));
	if(ref > 0)
	{
		atomic_decrement(&(sip->ref_count));
		return;
	}
	else if(ref < 0)
	{
		OSIP_TRACE(osip_trace
			(__FILE__, __LINE__, OSIP_FATAL, NULL,
			"sip msg ref MUST NOT less than zero!\n"));
		return;
	}
	...
	osip_free(sip->from_ip);
	osip_free(sip->outbound_ip);
	osip_free(sip);
}

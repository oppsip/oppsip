...
scond_t * scond_init(int val)
{
	scond_t *cond = (scond_t*)osip_malloc(sizeof(scond_t));

	if( 0 != pthread_mutex_init(&(cond->mut),NULL) )
	{
		osip_free(cond);
		return NULL;
	}
...
int scond_wait(scond_t *cond,int val)
{
	pthread_mutex_lock(&(cond->mut));

	while(cond->value != val)
...
int scond_try_wait(scond_t *cond,int val)
{
	int ret = -1;
...
int scond_signal(scond_t *cond,int cond_val,int new_val)
{
	int dosignal = 0;

	pthread_mutex_lock(&(cond->mut));

	if(cond->value == cond_val)
	{
		dosignal = 1;
		cond->value = new_val;
	}
...
int scond_destroy(scond_t *cond)
{
	if(cond == NULL)
		return -1;

...
int osip_sem_timedwait(struct osip_sem *sem,long milli_sec)
{
	struct timespec ts;
	...
	ts.tv_sec += milli_sec/1000;
	ts.tv_nsec += (milli_sec%1000) * 1000 * 1000;

	while(-1 == sem_timedwait(sem,&ts) && errno == EINTR)
		continue;

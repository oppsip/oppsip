...
char *new_random_string()
{
	int i;
	char *tmp = (char *)osip_malloc(33);
	for(i=0;i<4;i++)
		sprintf(tmp+8*i,"%08x",osip_build_random_number());
	return tmp;
}
...
{
	struct mem_list_s *next_mem; 
	struct mem_list_s *prev_mem; 
	int len;
}SIP_MEM_LIST;

static SIP_MEM_LIST *LAST_USED_PTR;//尾指针
static SIP_MEM_LIST first_used;//父节点

 int mem_alloc_count = 0;
 int mem_free_count = 0;

static osip_mutex_t *sg_mem_mutex = 0;

int my_memory_init()
{
	first_used.len=0;
	first_used.next_mem=0;
	first_used.prev_mem=0;

	LAST_USED_PTR = &first_used;

	//mem_alloc_count = 0;
	printf("my_memory_init\n");

	sg_mem_mutex = osip_mutex_init();
	if(sg_mem_mutex == NULL)
		return -1;
...
static void *my_malloc (size_t size)
{
	...
	if(value != NULL)
	{
		//新增块的next指针为空
		value->next_mem=NULL;
		value->len=	size-sizeof(SIP_MEM_LIST);

		osip_mutex_lock(sg_mem_mutex);

		//设新增块的prev指针为尾指针
		value->prev_mem=LAST_USED_PTR;

		//置前次分配的内存的next指针指向新增块
		LAST_USED_PTR->next_mem=value;

		//更新尾指针，指向新增块
		LAST_USED_PTR=(SIP_MEM_LIST *)value;

		mem_alloc_count++;

		//printf("osip_mem_alloc:%d\n",mem_alloc_count);

		osip_mutex_unlock(sg_mem_mutex);

		return (void *)(value+1);
	}
	else
	{
		//"<port_malloc.c> virtual memory exhausted\n"
		return NULL;
	}
}
...
static void my_free (void *ptr)
{
	...
	if (ptr != NULL)
	{
		value=(SIP_MEM_LIST *)ptr;
		value=value-1;

		osip_mutex_lock(sg_mem_mutex);

		if(value->next_mem==NULL)
		{//==尾指针?
			if(value==LAST_USED_PTR)
			{
				//尾指针前移
				LAST_USED_PTR=value->prev_mem;
			}
			else 
			{
				//只有尾指针的next_mem==NULL
			...
			//删除value节点
			value->prev_mem->next_mem=value->next_mem;
		}
		else
		{
			//删除value节点
			value->next_mem->prev_mem=value->prev_mem;
			value->prev_mem->next_mem=value->next_mem;
		}
...
static void *my_realloc (void *str,size_t size)
{
	...
	len=(((SIP_MEM_LIST *)str)-1)->len;

	/*
	len=((len+sizeof(SIP_MEM_LIST)-1)/SIP_MIN_BLOCK+1)*SIP_MIN_BLOCK;

	if(size <= (size_t)len) 
	{
	(((SIP_MEM_LIST *)str)-1)->len=size;
	return str;
	}*/
...
#define MEM_ALLOC_INFO_LEN (40)

void *_osip_malloc(size_t size, const char *file, long line)
{
	void *mem;
	//int len = 0;

	if(osip_malloc_func == NULL)
	{
		osip_set_allocators(my_malloc,my_realloc,my_free);
		my_memory_init();
	}

	mem = my_malloc(size + MEM_ALLOC_INFO_LEN);
	if (mem != NULL) {
		char *s;
		...
		//if(len > MEM_ALLOC_INFO_LEN-5)
		//	len = MEM_ALLOC_INFO_LEN-5;

		snprintf(mem,MEM_ALLOC_INFO_LEN-1,"%s-%d\n",s,line);
		*((char*)mem+MEM_ALLOC_INFO_LEN-1) = 0;

		//mem_alloc_count++;
		return (void *) ((char *) mem + MEM_ALLOC_INFO_LEN);
	}
	return NULL;
}
...
{
	if (ptr != NULL) {
		my_free((char *) ptr - MEM_ALLOC_INFO_LEN);
		//mem_alloc_count--;
	}
}

void *_osip_realloc(void *ptr, size_t size, const char *file, long line)
{
	void *mem;

	mem = my_realloc((char *) ptr - MEM_ALLOC_INFO_LEN, size + MEM_ALLOC_INFO_LEN);
	if (mem != NULL) {
		char *s;
		...
		*((char*)mem+MEM_ALLOC_INFO_LEN-1) = 0;
		...
		return (char *) mem + MEM_ALLOC_INFO_LEN;
	}
	return NULL;
}
...
int show_mem(int fd)
{
	...
	/*ptr= first_used.next_mem;

	while(ptr != 0)
	{
		write(fd,ptr+1,strlen((const char*)(ptr+1))+1);
		ptr=ptr->next_mem;
		x++;
	}*/

	sprintf(buf,"osip_malloc=%d %d %d %d\n",mem_alloc_count,mem_free_count,mem_alloc_count-mem_free_count,x);

	osip_mutex_unlock(sg_mem_mutex);

	write(fd,(const void*)buf,strlen(buf));
...
#ifdef LINUX_OS
#ifdef OSIP_MT
static osip_mutex_t *sg_atomic_increment_mutex;
#endif
#endif

int atomic_operator_init()
{
#ifdef WIN32
	return 0;
#elif defined(LINUX_OS)
#ifdef OSIP_MT
	sg_atomic_increment_mutex = osip_mutex_init();
#endif
	return 0;
#else
...
void atomic_operator_free()
{
#ifdef LINUX_OS
#ifdef OSIP_MT
	osip_mutex_destroy(sg_atomic_increment_mutex);
#endif
#endif

}

long atomic_get_value(long *lpCount)
{
	long lRet;
#ifdef OSIP_MT
	osip_mutex_lock(sg_atomic_increment_mutex);
#endif
	lRet = (*lpCount);
#ifdef OSIP_MT
	osip_mutex_unlock(sg_atomic_increment_mutex);
#endif
	return lRet;
}

long atomic_set_value(long *lpCount,long value)
{
	long lRet;
#ifdef OSIP_MT
	osip_mutex_lock(sg_atomic_increment_mutex);
...
long atomic_increment(long *lpCount)
{
#ifdef WIN32
	return InterlockedIncrement(lpCount);
#else
	long lRet;
#ifdef OSIP_MT	
	osip_mutex_lock(sg_atomic_increment_mutex);
...
long atomic_decrement(long *lpCount)
{
#ifdef WIN32
	return InterlockedDecrement(lpCount);
#else
	long lRet;
#ifdef OSIP_MT	
	osip_mutex_lock(sg_atomic_increment_mutex);
...
const char *osip_inet_ntop(SOCKADDR_IN *addr_in,char *addr,int addrLen)
{
#ifdef WIN32
	strncpy(addr,inet_ntoa(addr_in->sin_addr),addrLen);
	addr[addrLen-1] = 0;
	return addr;
#else
	return inet_ntop(AF_INET,addr_in,addr,addrLen);
#endif
}

int osip_inet_pton(const char *src, void *dst)
{
#ifdef WIN32
	((struct sockaddr_in*)dst)->sin_addr.s_addr = inet_addr(src);
	return 1;
#else
...
int MakeHex(const unsigned char *in, int inLen,char *out,int outLen)
{
	int i;
	if(outLen < 2*inLen+1)
		return -1;
	...
	for(i=0;i<inLen;i++)
	{
		...
		if((in[i] & 0x0f) < 10)
			out[i*2+1] = '0' + (in[i] & 0x0f);
		else
			out[i*2+1] = 'A' + (in[i] & 0x0f) - 10;
	}
